...
This results in impressive performance, but is not portable across architecture supported by ROOT.
...
The [initial version][initial] added to ROOT takes around 30ns per random `double`.
...
The graph below (created with ROOT) shows the evolution of performance after tuning.
