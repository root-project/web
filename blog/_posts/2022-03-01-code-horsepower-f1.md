...
### IDEs to the rescue
An Integrated Development Environment (IDE) is a very powerful tools to detect errors (thanks e.g. to [Clang](https://clang.llvm.org/)), trace them back to the right point in the source code, and even automatically suggest the solution. *ROOT* scripts, as well as standalone *C++* programs relying on *ROOT* libraries, can be integrated with minimum effort into these IDEs. Examples on the steps to follow are nicely explained in older blog posts for the [Visual Studio](https://root.cern/blog/root-on-vscode/) and [Eclipse](https://root.cern/blog/debugging-root-scripts-in-eclipse/) IDEs, as well as in the [Twiki](https://twiki.cern.ch/twiki/bin/view/CMSPublic/BristolEclipseTutorial) and [other blogs](
